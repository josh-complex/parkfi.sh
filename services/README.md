...
| `cron-profiles`       | `bun run cron:profiles`       | Cron `30 7 * * *`    | daily; one `REFRESH MATERIALIZED VIEW CONCURRENTLY attraction_hour_profile` — the 30-day average standby per attraction per park-local hour the Waits board reads for "35 under its usual for 3 PM" and its "Shortest later" column. Keyless. Plan: docs/plans/waits-redesign/README.md §5.2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `cron-public-records` | `bun run cron:public-records` | Cron `0 9 * * *`     | daily; government records → `public_record` ledger + entity links + revisions, served at `/filings`. Adapters: City of Orlando permits (Socrata, keyless; `SODA_APP_TOKEN` optional), USPTO trademarks (TDXF daily bulk) + patents (PFW search) — both need the free `USPTO_ODP_API_KEY` and skip themselves without it; FAA OE/AAA aeronautical studies near the parks (keyless region-year CSV archive), SFWMD environmental resource permit applications (keyless ArcGIS feed), FDACS exempt-facility ride incident report (weekly; one cumulative PDF parsed with `unpdf`, aggregated per attraction on public pages only). Ends by evaluating filing watches (§6.3) and enqueueing `filing-alerts` jobs — needs `REDIS_URL`. `--relink [--source=] [--since=] [--dry-run]` re-runs entity linking + scoring over the ledger instead of fetching (run after linking-rule or catalog changes). Plan: docs/plans/public-records-intelligence.md |
...
The `notifications` worker (`bun run notifications`, long-running) hosts the BullMQ
workers on one Redis connection: `push-notifications` (web push), `stay-alerts`
(durable, retried resort-availability EMAIL via Resend), `dining-alerts` (the same
for tables) and `filing-alerts` (public-records watch hits, as email and/or push).
Each is evaluated at the end of the sweep that could trigger it, which writes a
durable notification row (status `queued`) and enqueues a job carrying its id; the
worker renders a React Email template and sends it. Set `ALERTS_SEND_ENABLED=true`
to actually send — it defaults OFF so dev/test runs log instead of mailing.
