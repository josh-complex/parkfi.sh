 ...
 * machinery (and the selection/ride-filter providers it needs) live on `_app`
 * so the singleton `ParkMap` survives hops to non-dashboard sections — and so
 * does the achievement tracker now (it used to mount here, which disarmed the
 * native ride recorder on every hop to dining/stays/pins). This layer's only
 * remaining job is the parks prefetch loader so the dashboard hydrates instantly.
 */
function DashLayout() {
  return <Outlet />;
}
