...
| `cron-public-records` | `bun run cron:public-records` | Cron `0 9 * * *`     | daily; government records (City of Orlando permits via Socrata; more adapters per plan) → `public_record` ledger + entity links + revisions, served at `/filings`. Keyless (`SODA_APP_TOKEN` optional). Plan: docs/plans/public-records-intelligence.md |
