...
function ruleLabel(mode: number, priceBelow: number | null): string {
  return mode === BECOMES_AVAILABLE
    ? "When a room opens"
    : `When under $${(priceBelow ?? 0).toLocaleString()}/night`;
}
...
export function StayAlertsManager() {
  ...
  if (isPending) {
    return (
      <div className="mx-auto w-full max-w-2xl space-y-3 px-4 py-8">
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            We'll email you when a resort opens up or drops below your price. {alerts.length} of{" "}
            {limit} used.
          </p>
        </div>
